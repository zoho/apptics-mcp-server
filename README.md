...
| `list_applications` | Lists applications registered under a project, keyed by application identifier (`aaid`). |
| `download_config_file` | Downloads the Apptics SDK configuration file (e.g. `apptics-config.plist`) for an application by its `aaid`. |
